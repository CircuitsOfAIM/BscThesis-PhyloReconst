...
RAXML_NG_SCRIPT = "./raxml-ng-linux"
...
def all_SPR(ds_path):	
	...
	#wherer should be the msa_rampath in my computer??????
	
	msa_rampath = "/dev/shm/tmp" + ds_path.split(SEP)[-2]
	 #  to be on the safe side (even though other processes shouldn't be able to access it)
	...
	try:
		with open(msa_rampath, "w") as fpw:
			fpw.write(msa_str)  
			# don't write the msa string to a variable (or write and release it)
		msa_str = ''
		...
		############## generate all SPRs and compute it likelihoods #############
		...
		# insteda of giveing a parameter called outpath from All_spr() , I defiend the out path with the prefered standard of outpath_prune & outpath_rgft in lines 406 and 407

		df.to_csv(SUMMARY_PER_DS.format(ds_path, "prune", "br", ""))
		# 	# step_number is not defined
		df.to_csv(SUMMARY_PER_DS.format(ds_path, "rgft", "br","1"))
...
if __name__ == '__main__':
	...
	# # specified default path to the starting tree of the dataset 82
	# parser.add_argument('--dataset_path', '-ds', required=True)
	# # parser.add_argument('--dataset_path', '-ds',)
	# args = parser.parse_args()

	# dataset_path = args.dataset_path
	dataset_path = 'training_data/{}/'
	lsdir =[d for d in os.listdir('./training_data')]

	outpath_prune = SUMMARY_PER_DS.format(dataset_path, "prune", "br", "")
	#result of the the line 395 is a string that is : outpath_prune = '{dataset_path}ds_summary_prune_br_step{}'
