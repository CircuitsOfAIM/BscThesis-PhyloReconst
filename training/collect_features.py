...
def index_shared_features(dff, ind, edge, move_type, features_dicts_dict):
	...
	for subtype in ["p", "r"]:
		...
		# print('befooooooooooore')
		# print(FEATURES["ntaxa_{}".format(subtype)])
...
if __name__ == '__main__':
	...
	for d in lsdir:
		outpath_prune = SUMMARY_PER_DS.format(dataset_path.format(d), "prune", "br", '')
		outpath_rgft = SUMMARY_PER_DS.format(dataset_path.format(d), "rgft", "br", step_number)
		collect_features(dataset_path.format(d), step_number, outpath_prune, outpath_rgft)
		print('file'+d+'done')