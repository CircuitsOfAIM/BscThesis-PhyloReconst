...
def truncate(df):
	...
	#above is the default. i change kfold to existing value,1.
	kfold = len(groups_ids) if KFOLD==10 else KFOLD
...
if __name__ == '__main__':
	...
	#parser : self-defined parser arguments (command-line helper for user during runtim)
	# The arguments represent the source or the destination of the data that the command acts on. For example, the command cp, which is used to copy one or more files to a file or a directory, takes at least one source and one target:
